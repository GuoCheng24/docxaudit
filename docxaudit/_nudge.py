...
never more than twice in the lifetime of an install. `DOCXAUDIT_NO_NUDGE=1`
...
REPO = "GuoCheng24/docxaudit"
_SHOW_AT = (5, 25)          # run counts at which we say something
_ENV_OFF = "DOCXAUDIT_NO_NUDGE"


def _state_path():
    base = os.environ.get("XDG_STATE_HOME") or (Path.home() / ".local" / "state")
    return Path(base) / "docxaudit" / "usage.json"
...
def record_run():
    ...
    try:
        ...
        if n in _SHOW_AT:
            print(
                "\n── docxaudit has been useful " + str(n) + " times. If it saved you time,\n"
                "   a star helps other people find it: https://github.com/" + REPO + "\n"
                "   (silence this with DOCXAUDIT_NO_NUDGE=1)",
                file=sys.stderr,
            )
    except Exception:
        pass
